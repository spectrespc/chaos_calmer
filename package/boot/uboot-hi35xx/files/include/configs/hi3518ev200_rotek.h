...
#define CONFIG_BOOTARGS		"totalmem=$(totalmem) mem=$(osmem) ethaddr=$(ethaddr) phyaddru=$(phyaddru) phyaddrd=$(phyaddrd) sensor=$(sensor) linux_cmd=$(linux_cmd) console=ttyAMA0,115200 panic=20 root=/dev/mtdblock4 rootfstype=squashfs,jffs2 mtdparts=hi_sfc:192k(boot),64k(env),64k(tech),2048k(kernel),5120k(rootfs),-(rootfs_data)"
...
#define CONFIG_SYS_PROMPT  "BBC_2020 # "	/* Monitor Command Prompt */
